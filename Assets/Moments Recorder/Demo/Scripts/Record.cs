...
public class Record : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (!Directory.Exists(Application.dataPath + Path.DirectorySeparatorChar + "Capture"))
        {
            Directory.CreateDirectory(Application.dataPath + Path.DirectorySeparatorChar + "Capture");
        }
    ...
    {
        if (m_IsSaving)
            strToShow = "Progress Report : " + m_Progress.ToString("F2") + "%";

        if (!m_IsSaving && !string.IsNullOrEmpty(m_LastFile))
            strToShow = "Last File Saved : " + m_LastFile;
    }
